...
#
# Tests 74-80 require kms_provider='local'.  They SKIP in the default
# regression container (which uses kms_provider='vault'); this script
# provides a container where they all run and must PASS.
#
# Usage:
...
log_stage "WALLET REGRESSION TESTS (kms_provider=local, tests 73-80)"
...
if ! container_psql "$CONTAINER" -v ON_ERROR_STOP=1 -c \
        "CREATE EXTENSION pg_vault_tde;"; then
    log_error "WALLET: CREATE EXTENSION failed"
...
log_ok "WALLET REGRESSION COMPLETE: tests 73-80 all passed"
exit 0
