...
log_stage "REGRESSION TESTS (52 v1.4 + 20 v1.5 + 38 v1.6 + 23 v1.7  = 133 total)"
...
if container_psql "$CONTAINER" -f /tmp/regression_test_v17.sql; then
    ELAPSED=$(timer_elapsed "$START")
    log_ok "REGRESSION v1.7 wallet: tests 111-133 OK ($(timer_fmt "$ELAPSED"))"
else
    ELAPSED=$(timer_elapsed "$START")
...
log_ok "REGRESSION COMPLETE: ALL 133 TESTS PASSED (v1.4 × 52 + v1.5 × 20 + v1.6 × 38 + v1.7 × 23)"
exit 0
