...
log_stage "REGRESSION TESTS (52 v1.4 + 20 v1.5 + 38 v1.6 + 27 v1.7  = 137 total)"
...
log_info "Running v1.7 wallet regression_test_v17.sql (tests 111-137) ..."
START=$(timer_start)
if container_psql "$CONTAINER" -f /tmp/regression_test_v17.sql; then
    ELAPSED=$(timer_elapsed "$START")
    log_ok "REGRESSION v1.7 wallet: tests 111-137 OK ($(timer_fmt "$ELAPSED"))"
else
    ELAPSED=$(timer_elapsed "$START")
...
log_ok "REGRESSION COMPLETE: ALL 137 TESTS PASSED (v1.4 × 52 + v1.5 × 20 + v1.6 × 38 + v1.7 × 27)"
exit 0
