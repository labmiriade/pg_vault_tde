...
#   5  pg_vault_tde_health_check() shows vault_reachable=true
#   6  pg_vault_tde_health_check() shows auth_method=approle
...
# Test 5: health_check() shows vault_reachable=true
log_info "Test 5: health_check() vault_reachable=true ..."
...
if [[ "$health" == "t" ]]; then
    pass "Test 5: health_check vault_reachable=true"
else
    fail "Test 5: health_check vault_reachable returned: $health"
fi

# Test 6: health_check() shows auth_method=approle
log_info "Test 6: health_check() auth_method=approle ..."
...
if [[ "$auth_method" == "approle" ]]; then
    pass "Test 6: health_check auth_method=approle"
else
    fail "Test 6: health_check auth_method returned: $auth_method"
fi

# Test 7: KEK rewrap — advances key version in OpenBao Raft
log_info "Test 7: pg_vault_tde_vault_rewrap_dek() ..."
if $RT exec -u postgres pg-tde-bao psql -U postgres -At -c \
        "SELECT pg_vault_tde_vault_rewrap_dek();" > /dev/null 2>&1; then
    pass "Test 7: KEK rewrap succeeded"
else
    fail "Test 7: KEK rewrap failed"
fi
...
# Test 10: bao-2 follower is up and unsealed
bao_api_ok 10 "OpenBao bao-2 sealed=false (Raft follower)" "$BAO_PORT_2" "/v1/sys/health"

# Test 11: bao-3 follower is up and unsealed
bao_api_ok 11 "OpenBao bao-3 sealed=false (Raft follower)" "$BAO_PORT_3" "/v1/sys/health"
