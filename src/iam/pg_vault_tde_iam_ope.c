                          ...
                          ScanKey orderbys, int norderbys)
{
	...
	if (keys != NULL)
	{
		...
		{ 
            /*
             * ORE supports order comparison, so we intercept all 5 strategies:
             * 1: <, 2: <=, 3: =, 4: >=, 5: >
             */
				 ...
				 keys[i].sk_strategy <= BTGreaterStrategyNumber))
			{
				...
				/*
				 * FIX: Route by actual storage property (typlen),
				 * NOT solely based on whether it uses an enc_ops class identifier.
				 */
				if (att->attlen != -1 && att->attlen != -2)
				{
					/* Safe for scalar primitives: int4, int8, uuid, date, timestamptz */
					keys[i].sk_argument =
						tde_iam_ope_encrypt_fixed_type_datum(scan->indexRelation,
															 keys[i].sk_argument,
															 scan->indexRelation->rd_opcintype[col]);
				}
				else
				{
					/* Safe for variable-length structures: text, varchar, bytea */
					keys[i].sk_argument =
