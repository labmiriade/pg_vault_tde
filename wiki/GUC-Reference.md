...
| `max_encrypted_relations` | integer | `1024` | postmaster | Maximum number of independently-keyed `encrypted_heap` relations in the shared-memory DEK cache (range 64–65536), **cluster-wide**: entries are keyed by `(dbid, relid)`, so this is the sum across all databases. Increase if you have more than 1024 encrypted tables. |
